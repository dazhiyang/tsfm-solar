...
# User might want to change Context Length to 1024 or use a refined model.
CONTEXT_LENGTH = 512
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "..", "Data", "Forecasts", "TTM-R2")
...
def run_forecast_station(stn_code, model=None):
    ...
    print(f"\nForecasting for {stn_code} using {MODEL_NAME} (TTM-R2)...")
    ...
    valid_int_indices = [t for t in target_int_indices if t >= CONTEXT_LENGTH] # Default TTM-v1 (CONTEXT_LENGTH is 512)
    ...
    def run_inference_for_column(col_name, pred_col_name):
        ...
        for i in tqdm(range(0, len(valid_int_indices), BATCH_SIZE), desc=f"    {col_name}", leave=False):
            ...
            batch_preds = forecast[:, 0, 0].cpu().numpy()
...
def main():
    print(f"Initializing TTM-R2 ({MODEL_NAME}) on {DEVICE}...")
    try:
